...
raw_abundance.p <- 
viral_abundance %>%
  ggplot(aes(dmy(collection_date), (Sequences), 
  ...
  facet_grid(~Species)+
  geom_text(aes(label=`Number of individuals`, y = (Sequences) + 0.1),
            position = position_dodge(0.9),
            vjust = -0.9, colour="black")+
  scale_y_continuous(limits=c(0,3500000))+
  scale_x_date(limits =c(dmy("20/01/2017"), dmy("15/12/2017")))+
  ylab("Read abundance") + xlab("Time")+
  theme(legend.position = "none")

# ggsave("../Wytham_rodent_manuscript/figures/Raw_abundance_2021-11-30.pdf", height = 2, width = 6, plot = raw_abundance.p)
...
prop_viral_reads <- (sum(total_virus_reads$Sequences)/335917017)*100

sum(total_virus_reads$Sequences)

print(paste0("Percentage viral reads ", round(prop_viral_reads,digits = 2), "%"))

length(unique(all_virus.df$virus_family))

all.hist <-
all_virus_species.df %>%
  filter(viral_read_count/Sequences >= 1/1000000) %>%
...
all_virus_species.df %>%
  ...
  group_by(Interval) %>%
  summarise(count = n()) %>%
  ggplot(aes(x=Interval)) + geom_col(fill="grey", colour="black", aes(y = (count)/sum(count)))  + theme_bw() + xlab("Sampling period") + ylab("Proportion")  + ggtitle("All") + theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(limits=c(0,0.31))
...
verterb_virus_species.df %>%
  ...
  group_by(Interval) %>%
  summarise(count = n()) %>%
  ggplot(aes(x=Interval)) + geom_col(fill="grey", colour="black", aes(y = (count)/sum(count)))  + theme_bw() + xlab("Sampling period") + ylab("Proportion")  + ggtitle("Vertebrate-associated") + theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(limits=c(0,0.31))
...
phage_species.df %>%
  ...
  group_by(Interval) %>%
  summarise(count = n()) %>%
  ggplot(aes(x=Interval)) + geom_col(fill="grey", colour="black", aes(y = (count)/sum(count)))  + theme_bw() + xlab("Sampling period") + ylab("Proportion")  + ggtitle("Bacteriophage") + theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(limits=c(0,0.31))
...
# ggsave("../Wytham_rodent_manuscript/figures/prop_virus_detected_vs_number_of_intervals.pdf", combined_summary, height=5, width=8)
...
# filtering the contingency table for contigs where abundance is greater than 1 per million read threshold
...
contigency_table.s <-
    ...
    mutate(WTCHG_714232_72815161_AS_1 =
             ifelse((AS1_abundance <= (1 / 1000000)), 0, WTCHG_714232_72815161_AS_1)) %>%
    mutate(WTCHG_714232_72835163_AS_2 =
             ifelse((AS2_abundance <= (1 / 1000000)), 0, WTCHG_714232_72835163_AS_2)) %>%
    mutate(WTCHG_714232_72865166_AS_3 =
             ifelse((AS3_abundance <= (1 / 1000000)), 0, WTCHG_714232_72865166_AS_3)) %>%
    mutate(WTCHG_714232_71935169_AS_4 =
             ifelse((AS4_abundance <= (1 / 1000000)), 0, WTCHG_714232_71935169_AS_4)) %>%
    mutate(WTCHG_714232_71965172_AS_5 =
             ifelse((AS5_abundance <= (1 / 1000000)), 0, WTCHG_714232_71965172_AS_5)) %>%
    mutate(WTCHG_714232_72825162_MG_1 =
             ifelse((MG1_abundance <= (1 / 1000000)), 0, WTCHG_714232_72825162_MG_1)) %>%
    mutate(WTCHG_714232_72845164_MG_2 =
             ifelse((MG2_abundance <= (1 / 1000000)), 0, WTCHG_714232_72845164_MG_2)) %>%
    mutate(WTCHG_714232_72875167_MG_3 =
             ifelse((MG3_abundance <= (1 / 1000000)), 0, WTCHG_714232_72875167_MG_3)) %>%
    mutate(WTCHG_714232_71945170_MG_4 =
             ifelse((MG4_abundance <= (1 / 1000000)), 0, WTCHG_714232_71945170_MG_4)) %>%
    mutate(WTCHG_714232_71975173_MG_5 =
             ifelse((MG5_abundance <= (1 / 1000000)), 0, WTCHG_714232_71975173_MG_5)) %>%
    mutate(WTCHG_714232_72855165_AF_3 =
             ifelse((AF3_abundance <= (1 / 1000000)), 0, WTCHG_714232_72855165_AF_3)) %>%
    mutate(WTCHG_714232_72885168_AF_4 =
             ifelse((AF4_abundance <= (1 / 1000000)), 0, WTCHG_714232_72885168_AF_4)) %>%
    mutate(WTCHG_714232_71955171_AF_5 =
             ifelse((AF5_abundance <= (1 / 1000000)), 0, WTCHG_714232_71955171_AF_5)) %>%
...
get_venn_data <- function(blast_results_table, contingency_table, list) {
  ...
  contigency_table.s <-
    ...
    mutate(AS1 =
             ifelse((AS1_abundance <= (1 / 1000000)), 0, AS1)) %>%
    mutate(AS2 =
             ifelse((AS2_abundance <= (1 / 1000000)), 0, AS2)) %>%
    mutate(AS3 =
             ifelse((AS3_abundance <= (1 / 1000000)), 0, AS3)) %>%
    mutate(AS4 =
             ifelse((AS4_abundance <= (1 / 1000000)), 0, AS4)) %>%
    mutate(AS5 =
             ifelse((AS5_abundance <= (1 / 1000000)), 0, AS5)) %>%
    mutate(MG1 =
             ifelse((MG1_abundance <= (1 / 1000000)), 0, MG1)) %>%
    mutate(MG2 =
             ifelse((MG2_abundance <= (1 / 1000000)), 0, MG2)) %>%
    mutate(MG3 =
             ifelse((MG3_abundance <= (1 / 1000000)), 0, MG3)) %>%
    mutate(MG4 =
             ifelse((MG4_abundance <= (1 / 1000000)), 0, MG4)) %>%
    mutate(MG5 =
             ifelse((MG5_abundance <= (1 / 1000000)), 0, MG5)) %>%
    mutate(AF3 =
             ifelse((AF3_abundance <= (1 / 1000000)), 0, AF3)) %>%
    mutate(AF4 =
             ifelse((AF4_abundance <= (1 / 1000000)), 0, AF4)) %>%
    mutate(AF5 =
             ifelse((AF5_abundance <= (1 / 1000000)), 0, AF5)) %>%
...
# ggsave(filename = "../Wytham_rodent_manuscript/figures/venn_virus_contigs_by_species_and_virus_groups.pdf", contigs.venn)
...
get_venn_data_genera <-
  function(blast_results_table,
    ...
    mutate(AS1 =
             ifelse((AS1_abundance <= (2 / 1000000)), 0, AS1)) %>%
    mutate(AS2 =
             ifelse((AS2_abundance <= (2 / 1000000)), 0, AS2)) %>%
    mutate(AS3 =
             ifelse((AS3_abundance <= (2 / 1000000)), 0, AS3)) %>%
    mutate(AS4 =
             ifelse((AS4_abundance <= (2 / 1000000)), 0, AS4)) %>%
    mutate(AS5 =
             ifelse((AS5_abundance <= (2 / 1000000)), 0, AS5)) %>%
    mutate(MG1 =
             ifelse((MG1_abundance <= (2 / 1000000)), 0, MG1)) %>%
    mutate(MG2 =
             ifelse((MG2_abundance <= (2 / 1000000)), 0, MG2)) %>%
    mutate(MG3 =
             ifelse((MG3_abundance <= (2 / 1000000)), 0, MG3)) %>%
    mutate(MG4 =
             ifelse((MG4_abundance <= (2 / 1000000)), 0, MG4)) %>%
    mutate(MG5 =
             ifelse((MG5_abundance <= (2 / 1000000)), 0, MG5)) %>%
    mutate(AF3 =
             ifelse((AF3_abundance <= (2 / 1000000)), 0, AF3)) %>%
    mutate(AF4 =
             ifelse((AF4_abundance <= (2 / 1000000)), 0, AF4)) %>%
    mutate(AF5 =
             ifelse((AF5_abundance <= (2 / 1000000)), 0, AF5)) %>%
...
# ggsave(filename = "../Wytham_rodent_manuscript/figures/venn_virus_family.pdf", virusfamily_venn)
...
##### all viruses #####
all_virus_relabund <- get_relAbundance_s(unique(all_virus_species.df$virus_species), contigency_table.s)
...
ggplot(aes(samples, species_richness)) + 
  ...
  theme_bw() + xlab("Sample number") + ylab("Species richness") + ggtitle("All") + theme(plot.title = element_text(hjust = 0.5))
...
vert_virus_relabund <- get_relAbundance_s(unique(verterb_virus_species.df$virus_species), contigency_table.s)
...
ggplot(aes(samples, species_richness)) + 
  geom_line() + geom_point() +
  geom_ribbon(aes(ymin=species_richness-sd, ymax=species_richness+sd), alpha=0.2) + 
  theme_bw() + xlab("Sample number") + ylab("Species richness") + ggtitle("Vertebrate-associated") + theme(plot.title = element_text(hjust = 0.5))

spec_accum_curve.vert

##### bacteriophage #####

phage_relabund <- get_relAbundance_s(unique(phage_species.df$virus_species), contigency_table.s)
...
ggplot(aes(samples, species_richness)) + 
  geom_line() + geom_point() +
  geom_ribbon(aes(ymin=species_richness-sd, ymax=species_richness+sd), alpha=0.2) + 
  theme_bw() + xlab("Sample number") + ylab("Species richness") + ggtitle("Bacteriophage") + theme(plot.title = element_text(hjust = 0.5))
...
ggsave("../Wytham_rodent_manuscript/figures/rarefraction_curves_2021-12-10.pdf", spec_accum_plots, height=2.5, width=8)
...
picorna_abundance_over_time <-
  ...
  get_relAbundance_s(unique(picorna$species_list), contigency_table.s)  %>% 
  mutate(viral_read_count =
           ifelse((viral_read_count/Sequences <= (1/1000000)), 0, viral_read_count)) %>%
  # mutate(viral_read_count = 
  ...
  filter(virus_species %in% c("Mosavirus", 
    ...
    ggplot(aes(dmy(collection_date), 
             ...
             y = viral_read_sum/`Number of individuals`)) +
  geom_point() + 
  geom_line() + 
...
ggsave("../Wytham_rodent_manuscript/figures/picornavirus_abundance_2021-11-30.pdf", picorna_abundance_over_time, height=4, width=6)
...
levels <- data.frame(i1 = i1, i3 = i3, i4 = i4)
levels.2 <- data.frame(i5 = i5, i6 = i6, i4 = i4)
...
comm_mat <-
all_virus_species.df %>%
  mutate(viral_read_count = ifelse(viral_read_count/Sequences <= 1/1000000, 0, viral_read_count)) %>%
  get_comm_mat_s()
...
ggsave("../Wytham_rodent_manuscript/figures/picornavirus_diversity_2021-11-30.pdf", picorna.fig, height=4.5, width=6)
picorna.fig

```


# Four most common families

```{r}

four_most_common_families <- read_csv("mapped_reads_four_most_common_families_2021-12-02.csv")

subset_most_common_families <-
four_most_common_families %>%
  left_join(total_virus_reads) %>%
  mutate(Interval = as.character(Interval)) %>%
  left_join(samples_per_pool) %>%
  select(`Virus family`, `Mapped read count`, Species, Interval, collection_date, `Number of individuals`, Sequences) %>%
  mutate(Normalised_abundance = `Mapped read count`/`Number of individuals`) %>%
  filter(Species %in% c("AS", "MG"))

picob <- subset_most_common_families %>%
  filter(`Virus family` %in% c("Picobirnaviridae")) %>%
  ggplot(aes(dmy(collection_date), Normalised_abundance)) + facet_grid(Species~`Virus family`) + geom_point() + geom_line() + theme_bw() + xlab("Time") + ylab("Normalised read abundance") + scale_y_log10()

levi <- subset_most_common_families %>%
  filter(`Virus family` %in% c("Leviviridae")) %>%
  ggplot(aes(dmy(collection_date), Normalised_abundance)) + facet_grid(Species~`Virus family`) + geom_point() + geom_line() + theme_bw() + xlab("Time") + ylab("Normalised read abundance") + scale_y_log10()

micro <- subset_most_common_families %>%
  filter(`Virus family` %in% c("Microviridae")) %>%
  ggplot(aes(dmy(collection_date), Normalised_abundance)) + facet_grid(Species~`Virus family`) + geom_point() + geom_line() + theme_bw() + xlab("Time") + ylab("Normalised read abundance") + scale_y_log10()

picorna_a <- subset_most_common_families %>%
  filter(`Virus family` %in% c("Picornaviridae")) %>%
  ggplot(aes(dmy(collection_date), Normalised_abundance)) + facet_grid(Species~`Virus family`) + geom_point() + geom_line() + theme_bw() + xlab("Time") + ylab("Normalised read abundance") + scale_y_log10()

abundance.p <- (picob | levi | micro | picorna_a)

ggsave("../Wytham_rodent_manuscript/figures/abundance_pattern_of_most_common_families_v3.pdf", abundance.p, height=3.5, width=12)
``` 
...
# write_csv(timeseries_df, "../Wytham_rodent_manuscript/cross-correlation_analysis/imputed_timeseries_data_Wytham_rodent_virome.csv")
...
timeseries_df %>%
  ...
  mutate(variable = ifelse(variable == "diversity", "Shannon.diversity", variable)) %>%
  mutate(variable = ifelse(variable == "rich", "Species.richness", variable)) %>%
...
observed_data <-
diversity_stats %>%
  select(Collection.date, Shannon.diversity, Species.richness, Species) %>%
  filter(Species %in% c("AS", "MG")) %>%
...
combined_df$variable <- factor(combined_df$variable, levels=c("Shannon.diversity", "Species.richness", "MNKA_per_ha"))
...
combined_plot <- ggarrange(fitted_vs_observed, weather_plot,heights = c(1, 0.6), nrow = 2)
ggsave("../Wytham_rodent_manuscript/figures/Figure_SX_fitted_vs_observed_host_species_metrics.pdf", combined_plot, height = 8, width=10)
...
ccf_values_rich_AS1 # temp_mean; max sig lag = -1 (0.829)
ccf_values_rich_AS2 # temp_max; max sig lag = -1 (0.826)
ccf_values_rich_AS3 # hum; max sig lag = 0 (-0.891)
ccf_values_rich_AS4 # rain; max sig lag = -5 (0.401)
ccf_values_rich_AS5 # rain_wk; max sig lag = -6 (0.527)
ccf_values_rich_AS6 # density; max sig lag = 0 (-0.519)
...
ccf_values_rich_MG1 # temp_mean; max sig lag = -2 (0.773)
ccf_values_rich_MG2 # temp_max; max sig lag = -2 (0.775)
ccf_values_rich_MG3 # hum; max sig lag = 0 (-0.820)
ccf_values_rich_MG4 # rain; max sig lag = -6 (0.403)
ccf_values_rich_MG5 # rain_wk; max sig lag = -7 (0.494)
ccf_values_rich_MG6 # density; max sig lag = 0 (-0.409)
...
MG_diversity_predictors <- data.frame(temp_3 = predictor_df$temp_3, 
                                      hum = predictor_df$hum,
                                      rain_7 = predictor_df$rain_7,
                                      MG_density_5 = predictor_df$MG_density_5)


AS_rich_predictors <- data.frame(temp_1 = predictor_df$temp_1, 
                                 hum = predictor_df$hum,
                                 rain_5 = predictor_df$rain_5,
                                 AS_density = predictor_df$AS_density)


MG_rich_predictors <- data.frame(temp_2 = predictor_df$temp_2, 
                                 hum = predictor_df$hum,
                                 rain_6 = predictor_df$rain_6,
...
corrplot(AS_diversity_M, method = 'number') #(rain+hum)
corrplot(MG_diversity_M, method = 'number') #(rain+hum) + (density+temp)
corrplot(AS_rich_M, method = 'number') # temp+hum + rain+density
corrplot(MG_rich_M, method = 'number') # hum+density
...
# MG diversity -> temp_3 + density_5 correlated (0.89) and hum and rain_7 correlated (0.76)
# temp + hum
# hum + density
# temp + rain
# rain + density

# AS rich -> rain and density correlated (-0.73) + temp+hum (-0.79)
# temp + rain
# temp + density
# hum + density
# rain + hum
...
# MG rich -> hum and density correlated (-0.83)
# temp + hum + rain
# temp + density + rain
# temp + density
# temp + rain
...
MG_div_model_a <- glm(data = predictor_df, formula = MG_diversity ~ temp_3 + hum, family = gaussian)
MG_div_model_b <- glm(data = predictor_df, formula = MG_diversity ~ hum + MG_density_5, family = gaussian) ###
MG_div_model_c <- glm(data = predictor_df, formula = MG_diversity ~ temp_3 + rain_7, family = gaussian)
MG_div_model_d <- glm(data = predictor_df, formula = MG_diversity ~ rain_7 + MG_density_5, family = gaussian)


#AS_rich 

AS_rich_model_a <- glm(data = predictor_df, formula = AS_rich ~ temp_1 + rain_5, family = gaussian)
AS_rich_model_b <- glm(data = predictor_df, formula = AS_rich ~ rain_5 + hum, family = gaussian) ###
AS_rich_model_c <- glm(data = predictor_df, formula = AS_rich ~ temp_1 + AS_density, family = gaussian)
AS_rich_model_d <- glm(data = predictor_df, formula = AS_rich ~ hum + AS_density, family = gaussian)


#MG_rich 

MG_rich_model_a <- glm(data = predictor_df, formula = MG_rich ~ temp_2 + hum + rain_6, family = gaussian)
MG_rich_model_b <- glm(data = predictor_df, formula = MG_rich ~temp_2 + MG_density + rain_6, family = gaussian) ###
MG_rich_model_c <- glm(data = predictor_df, formula = MG_rich ~ temp_2 + MG_density, family = gaussian)
MG_rich_model_d <- glm(data = predictor_df, formula = MG_rich ~ temp_2 + rain_6, family = gaussian)
MG_rich_model_e <- glm(data = predictor_df, formula = MG_rich ~ temp_2 + hum, family = gaussian)
MG_rich_model_f <- glm(data = predictor_df, formula = MG_rich ~  hum + rain_6, family = gaussian)
MG_rich_model_g <- glm(data = predictor_df, formula = MG_rich ~  rain_6 + MG_density, family = gaussian)
...
MG_rich_models <- list(MG_rich_model_a, MG_rich_model_b, 
                      ...
                      MG_rich_model_e, MG_rich_model_f, MG_rich_model_g)
model.names <- c("model_a", "model_b", "model_c", "model_d", "model_e", "model_f", "model_g")
...
MG_div_fit <- MG_div_model_b
AS_rich_fit <- AS_rich_model_b
MG_rich_fit <- MG_rich_model_b
...
ggarrange(sd, sr, common.legend = TRUE, 
          labels = c("Shannon diversity", "Species richness"))
